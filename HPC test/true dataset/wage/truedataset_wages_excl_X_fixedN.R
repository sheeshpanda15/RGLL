...
# ── 过滤：取所有分组方案的有效索引交集，保证公共集对每个方案都满足约束 ──
all_schemes <- list(
  "R8496500",
  "R7898500",
  "R8497000",
  ...
  c("R7898500", "R8496500"),
  c("R8497000", "R7898500"),
  c("R8497000", "R8496500", "R7898500")
)

for (scheme in all_schemes) {
  repeat {
    if (length(scheme) == 1L) {
      lbl_tr <- as.character(dat[[scheme]][train_global])
      lbl_te <- as.character(dat[[scheme]][test_global])
    } else {
      lbl_tr <- do.call(paste, c(lapply(scheme, function(col) as.character(dat[[col]][train_global])), sep = "_"))
      lbl_te <- do.call(paste, c(lapply(scheme, function(col) as.character(dat[[col]][test_global])),  sep = "_"))
    }
    vb <- intersect(names(which(table(lbl_tr) >= 2)),
                    names(which(table(lbl_te) >= 1)))
    kt <- lbl_tr %in% vb
    ke <- lbl_te %in% vb
    if (all(kt) && all(ke)) break
    train_global <- train_global[kt]
    test_global  <- test_global[ke]
  }
}

cat("公共 Train N:", length(train_global),
    "| 公共 Test N:", length(test_global), "\n\n")
...
make_group_data_multi <- function(group_cols,
  ...
  # ── 2. 直接使用公共索引，不再过滤（公共集已在外部对所有方案统一过滤）
  idx_train <- train_global
  idx_test  <- test_global
  lbl_tr    <- raw_label_train
  lbl_te    <- raw_label_test
...
# ── 新增多列组合分组 ─────────────────────────────────────────
d_edu_region   <- make_group_data_multi(c("R8497000", "R8496500"))
d_worker_region   <- make_group_data_multi(c("R7898500", "R8496500"))
d_edu_worker   <- make_group_data_multi(c("R8497000", "R7898500"))
d_edu_region_worker   <- make_group_data_multi(c("R8497000", "R8496500","R7898500"))# 教育×地区,  最多 18×4  组
...
all_datasets <- list(
  ...
  d_worker_region =d_worker_region,
  d_edu_worker   =d_edu_worker,
  d_edu_region_worker =d_edu_region_worker
)
