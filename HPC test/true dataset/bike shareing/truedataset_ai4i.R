...
# ── 读取数据 ───────────────────────────────────────────────
dat_raw <- read.csv("AirQualityUCI.csv", sep = ";", dec = ",", header = TRUE)

# 删除末尾两个空列和空行（Date为空的行）
# 删除因多余分号产生的自动命名空列
dat_raw$X <- NULL
dat_raw$X.1 <- NULL
dat_raw <- dat_raw[!is.na(dat_raw$Date) & dat_raw$Date != "", ]

# 把所有 -200 替换为 NA，再删除含 NA 的行
# 同时排除 NMHC.GT.（缺失率>90%）
dat_raw$NMHC.GT. <- NULL
num_cols <- setdiff(colnames(dat_raw), c("Date", "Time"))
for (col in num_cols) {
  dat_raw[[col]][dat_raw[[col]] <= -200] <- NA
}
dat_raw <- dat_raw[complete.cases(dat_raw), ]
cat("清洗后样本量:", nrow(dat_raw), "\n")

# ── 解析时间字段，提取小时和月份 ──────────────────────────
dat_raw$hour  <- as.integer(sub("^(\\d+)\\..*", "\\1", dat_raw$Time))
dat_raw$month <- as.integer(sub("^\\d+/(\\d+)/.*", "\\1",
                                sub("^(\\d+)/(\\d+)/(\\d+)$", "\\1/\\2/\\3",
                                    dat_raw$Date)))

# 修正：支持 DD/MM/YYYY 格式
date_parts     <- strsplit(dat_raw$Date, "/")
dat_raw$month  <- as.integer(sapply(date_parts, `[`, 2))

# ── 分组变量（3组：低/中/高 或 早/中/晚） ───────────────────
make_3group <- function(x) {
  q1 <- quantile(x, 0.25); q3 <- quantile(x, 0.75)
  as.integer(ifelse(x <= q1, 1L, ifelse(x >= q3, 3L, 2L)))
}

# 小时分3组：夜间(0-7)=1，白天(8-17)=2，傍晚(18-23)=3
dat_raw$hour_grp  <- as.integer(ifelse(dat_raw$hour <= 7, 1L,
                                       ifelse(dat_raw$hour <= 17, 2L, 3L)))

# 月份分3组：冬(12,1,2)=1，春夏(3-8)=2，秋(9-11)=3
dat_raw$month_grp <- as.integer(ifelse(dat_raw$month %in% c(12,1,2), 1L,
                                       ifelse(dat_raw$month %in% 3:8,  2L, 3L)))

dat_raw$T_grp   <- make_3group(dat_raw$T)
dat_raw$RH_grp  <- make_3group(dat_raw$RH)
dat_raw$AH_grp  <- make_3group(dat_raw$AH)

# ── 提取 y 和 X ────────────────────────────────────────────
# y：CO(GT) 真实浓度，标准化
y <- as.numeric(dat_raw$CO.GT.)
y <- (y - mean(y)) / sd(y)

exclude_cols <- c("Date", "Time", "CO.GT.",
                  "hour", "month",
                  "hour_grp", "month_grp",
                  "T_grp", "RH_grp", "AH_grp")
X_cols <- setdiff(colnames(dat_raw), exclude_cols)
X      <- as.matrix(dat_raw[, X_cols])
cat("X 列数 p:", ncol(X), "\n")
...
cat("Train N:", length(train_global), "| Test N:", length(test_global), "\n\n")

X_train <- X[train_global, ]; X_test <- X[test_global, ]
y_train <- y[train_global];   y_test <- y[test_global]

# ================================================================
# make_group_data_multi（不过滤任何行）
# ================================================================
make_group_data_multi <- function(group_cols) {
  if (length(group_cols) == 1L) {
    raw_label_train <- as.character(dat_raw[[group_cols]][train_global])
    raw_label_test  <- as.character(dat_raw[[group_cols]][test_global])
  } else {
    raw_label_train <- do.call(paste, c(
      lapply(group_cols, function(col) as.character(dat_raw[[col]][train_global])), sep = "_"))
    raw_label_test  <- do.call(paste, c(
      lapply(group_cols, function(col) as.character(dat_raw[[col]][test_global])), sep = "_"))
  }
  
  # 合并 train+test 统一建 factor，保证两边整数编码一致
  all_labels <- factor(c(raw_label_train, raw_label_test))
  g_all      <- as.integer(all_labels)
  g_train    <- g_all[seq_along(train_global)]
  g_test     <- g_all[seq_along(test_global) + length(train_global)]
  
  ord_train <- order(g_train); g_train_s <- g_train[ord_train]
  ord_test  <- order(g_test);  g_test_s  <- g_test[ord_test]
  
  list(
    FXX.train    = X_train[ord_train, , drop = FALSE],
    FY.train     = as.matrix(y_train[ord_train]),
    nc.train     = as.integer(table(g_train_s)),
    FXX.test     = X_test[ord_test,  , drop = FALSE],
    FY.test      = as.matrix(y_test[ord_test]),
    nc.test      = as.integer(table(g_test_s)),
    R            = length(unique(g_train_s)),
    p            = ncol(X_train),
    group_cols   = group_cols,
    group_labels = levels(all_labels)
  )
}
...
# ── 分组方案 ───────────────────────────────────────────────
d_hour      <- make_group_data_multi("hour_grp")          # 时段,     3组
d_month     <- make_group_data_multi("month_grp")         # 季节,     3组
d_T         <- make_group_data_multi("T_grp")             # 气温,     3组
d_RH        <- make_group_data_multi("RH_grp")            # 相对湿度, 3组
d_AH        <- make_group_data_multi("AH_grp")            # 绝对湿度, 3组

d_hour_month  <- make_group_data_multi(c("hour_grp",  "month_grp"))  # 时段×季节,   最多9组
d_hour_T      <- make_group_data_multi(c("hour_grp",  "T_grp"))      # 时段×气温,   最多9组
d_hour_RH     <- make_group_data_multi(c("hour_grp",  "RH_grp"))     # 时段×湿度,   最多9组
d_month_T     <- make_group_data_multi(c("month_grp", "T_grp"))      # 季节×气温,   最多9组
d_T_RH        <- make_group_data_multi(c("T_grp",     "RH_grp"))     # 气温×湿度,   最多9组

cat("=== 分组信息 ===\n")
all_datasets <- list(
  d_hour     = d_hour,     d_month    = d_month,
  d_T        = d_T,        d_RH       = d_RH,       d_AH = d_AH,
  d_hour_month = d_hour_month, d_hour_T = d_hour_T,
  d_hour_RH  = d_hour_RH,  d_month_T  = d_month_T,  d_T_RH = d_T_RH
)
...
mspe_summary <- do.call(rbind, lapply(names(results), function(nm) {
  ...
  data.frame(dataset=nm, R=all_datasets[[nm]]$R, p=all_datasets[[nm]]$p,
             mspe_ALL=r[[2]][1], mspe_GALL=r[[2]][2], mspe_GALLRS=r[[2]][3])
}))
print(mspe_summary)
