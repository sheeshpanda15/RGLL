...
# ── 读取数据（day 级别）──────────────────────────────────────
dat <- read.csv("day.csv", header = TRUE)
cat("样本量:", nrow(dat), "\n")

# ── 提取 y 和 X ────────────────────────────────────────────
# y = cnt（总租车量），标准化
y <- as.numeric(dat$cnt)
y <- (y - mean(y)) / sd(y)

# 排除：y 及其组成部分（casual/registered）、ID 列、日期列
# 分组变量（season/yr/mnth/holiday/weekday/workingday/weathersit）保留进 X
exclude_cols <- c("cnt", "casual", "registered", "instant", "dteday")
...
# 注意：day 数据集没有 hr 列
all_group_cols <- c("season", "yr", "mnth", "holiday", "weekday", "workingday", "weathersit")
...
# ── 分组方案（day 数据集无 hr 列）──────────────────────────
# 单列
d_season     <- make_group_data_multi("season")      # 季节,          4 组
d_yr         <- make_group_data_multi("yr")          # 年份,          2 组
d_mnth       <- make_group_data_multi("mnth")        # 月份,         12 组
d_weekday    <- make_group_data_multi("weekday")     # 星期,          7 组
d_workingday <- make_group_data_multi("workingday")  # 工作日,        2 组
d_weathersit <- make_group_data_multi("weathersit")  # 天气,          4 组

# 两两交互
d_season_mnth        <- make_group_data_multi(c("season", "mnth"))        # 季节×月份,   最多 4×12=48  组
d_season_weekday     <- make_group_data_multi(c("season", "weekday"))     # 季节×星期,   最多 4×7=28   组
d_season_weathersit  <- make_group_data_multi(c("season", "weathersit"))  # 季节×天气,   最多 4×4=16   组
d_mnth_weekday       <- make_group_data_multi(c("mnth", "weekday"))       # 月份×星期,   最多 12×7=84  组
d_mnth_weathersit    <- make_group_data_multi(c("mnth", "weathersit"))    # 月份×天气,   最多 12×4=48  组
d_weekday_weathersit <- make_group_data_multi(c("weekday", "weathersit")) # 星期×天气,   最多 7×4=28   组
...
all_datasets <- list(
  ...
  d_mnth        = d_mnth,
  d_weekday     = d_weekday,
  d_workingday  = d_workingday,
  d_weathersit  = d_weathersit,
  # 两两交互
  d_season_mnth        = d_season_mnth,
  d_season_weekday     = d_season_weekday,
  d_season_weathersit  = d_season_weathersit,
  d_mnth_weekday       = d_mnth_weekday,
  d_mnth_weathersit    = d_mnth_weathersit,
  d_weekday_weathersit = d_weekday_weathersit
)
