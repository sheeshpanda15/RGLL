...
# 含糖量分组（干型/半干/甜型）
dat$sugar_group <- ifelse(dat$residual.sugar < 4, 1L, ifelse(dat$residual.sugar < 12, 2L, 3L))
cat("合并后样本量:", nrow(dat), "\n")

# ── 提取 y 和 X ────────────────────────────────────────────
# y = quality（酒的评分，作为响应变量）
y <- as.numeric(dat$quality)
y <- (y - mean(y)) / sd(y)

# X = 除 quality、wine_type、sugar_group 之外的所有列
exclude_cols <- c("quality", "wine_type", "sugar_group")
...
group_cols <- c("quality", "wine_type", "sugar_group")
...
d_sugar    <- make_group_data("sugar_group") # 含糖量分组, 3组

cat("=== 分组信息 ===\n")
for (name in c("d_quality", "d_winetype", "d_sugar")) {
  d <- get(name)
...
# ── 运行 ───────────────────────────────────────────────────
result <- Comp(d_winetype, obj.c = 0.1)
result
