...
// --------- helper: scale each column to [-1, 1] -------------
...
Rcpp::List find_sigma_cpp(const arma::mat& xx,
  ...
  const double NF = static_cast<double>(offset);
  double NS = 0.0;
...
Rcpp::List Est_hat_cpp(const arma::mat& xx_in,
  ...
  // ── 收敛迭代（替换固定 3 轮）──
  const double tol_ri  = 1e-5;
  const int    max_ri  = 100;
  for (int i = 0; i < max_ri; ++i) {
    double Va_old = Var_a_curr, Ve_old = Var_e_curr;

    Rcpp::List sigs = find_sigma_ri_internal(yy, X, beta_curr,
                                             Var_a_curr, Var_e_curr, nc);
    ...
    if (std::abs(Var_a_curr - Va_old) < tol_ri &&
        std::abs(Var_e_curr - Ve_old) < tol_ri) break;
  }
...
// ======== RS helper 3: EM sigma (with posterior variance correction) ========
...
Rcpp::List count_info_rs_cpp(const arma::mat& xx_in,
  ...
  // ── 收敛迭代（替换固定 3 轮） ──
  // count_info_rs_cpp 在 SA 每步都被调用，使用稍宽松的 tol 换取速度
  const double tol_rs  = 1e-4;
  const int    max_rs  = 200;
  for (int it = 0; it < max_rs; ++it) {
    arma::mat G_old  = G_curr;
    double    Ve_old = Ve_curr;

    Rcpp::List sigs = rs_count_find_sigma_internal(xx, yy, beta_curr,
                                                   G_curr, Ve_curr, nc, p);
    ...
    double dG  = arma::norm(G_curr - G_old, "fro");
    double dVe = std::abs(Ve_curr - Ve_old);
    if (dG < tol_rs && dVe < tol_rs) break;
  }

  // ----- 构建信息矩阵 -----
  arma::mat infor(q, q, arma::fill::zeros);
...
Rcpp::List Est_hat_RS_cpp(const arma::mat& xx_in,
  ...
  // ----- 初始值 -----
  arma::mat D_mat(xx.n_rows, q, arma::fill::ones);
  D_mat.cols(1, p) = xx;
  arma::vec beta_curr = arma::solve(D_mat, yy);
  arma::mat G_curr    = arma::eye(q, q) * 0.1;
  double    Ve_curr   = 1.0;

  // ── 收敛迭代（替换固定 3 轮） ──
  const double tol_rs = 1e-5;
  const int    max_rs = 200;
