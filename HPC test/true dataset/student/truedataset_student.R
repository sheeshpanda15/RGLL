...
exclude_cols <- c("G3", "G1", "G2", "subject")
...
group_cols <- c("subject", "sex", "school", "address", "Pstatus", "Medu", "Fedu", "studytime", "reason", "goout")
...
# ── 分组方案 ───────────────────────────────────────────────
d_Medu      <- make_group_data("Medu")       # 母亲教育,  5组
d_Fedu      <- make_group_data("Fedu")       # 父亲教育,  5组
...
cat("=== 分组信息 ===\n")
for (name in c("d_Medu","d_Fedu","d_studytime","d_reason","d_goout")) {
  d <- get(name)
