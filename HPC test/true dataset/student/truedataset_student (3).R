...
# X = 除 G3、G1、G2、subject 之外的列
# G1/G2 排除：它们和G3高度相关会造成数据泄露
...
group_cols <- c("subject", "sex", "school", "address", "Pstatus", "studytime", "reason", "goout", "parent_edu")
...
# ── 分组方案 ───────────────────────────────────────────────
d_subject <- make_group_data("subject")   # 科目,   2组（葡语/数学）
d_sex     <- make_group_data("sex")       # 性别,   2组
d_school  <- make_group_data("school")    # 学校,   2组（GP/MS）
d_address <- make_group_data("address")   # 城乡,   2组（城市/农村）
d_Pstatus   <- make_group_data("Pstatus")    # 父母状态,  2组
d_studytime <- make_group_data("studytime")  # 每周学习,  4组
d_reason    <- make_group_data("reason")     # 选校原因,  4组
d_goout      <- make_group_data("goout")       # 外出频率,    5组
d_parent_edu <- make_group_data("parent_edu")  # 父母教育总和, 7组（2-8）

cat("=== 分组信息 ===\n")
for (name in c("d_subject","d_sex","d_school","d_address","d_Pstatus","d_studytime","d_reason","d_goout","d_parent_edu")) {
  d <- get(name)
...
# ── 运行 ───────────────────────────────────────────────────
result <- Comp(d_parent_edu, obj.c = 0.1)
result
