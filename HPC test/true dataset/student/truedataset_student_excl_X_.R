...
Comp=function(dataset,obj.c=0.5){
  ...
  group_params.curr <- cluster.curr$group_params  # 新增：保存当前聚类参数
  ...
  group_params.best <- group_params.curr  # 新增：保存最优聚类参数
  ...
  repeat {
    ...
    if (accept) {
      ...
      group_params.curr <- cluster.candi$group_params  # 新增：更新当前聚类参数
      ...
      if (obj.candi > obj.best) {
        ...
        group_params.best <- group_params.curr  # 新增：更新最优聚类参数
      }
    }
    T.curr <- T.curr * alpha
...
exclude_cols <- c("G3", "G1", "G2", "subject", "parent_edu",
                  "sex", "school", "address", "Pstatus",
                  "studytime", "reason", "goout", "Medu", "Fedu")
...
all_group_cols <- c("studytime","reason","goout","parent_edu")
...
# 两两交互
d_studytime_reason     <- make_group_data_multi(c("studytime", "reason"))       # 学习×原因,   最多 4×4=16  组
d_studytime_goout      <- make_group_data_multi(c("studytime", "goout"))        # 学习×外出,   最多 4×5=20  组
d_studytime_parent_edu <- make_group_data_multi(c("studytime", "parent_edu"))   # 学习×父母,   最多 4×7=28  组
d_reason_goout         <- make_group_data_multi(c("reason", "goout"))           # 原因×外出,   最多 4×5=20  组
d_reason_parent_edu    <- make_group_data_multi(c("reason", "parent_edu"))      # 原因×父母,   最多 4×7=28  组
d_goout_parent_edu     <- make_group_data_multi(c("goout", "parent_edu"))       # 外出×父母,   最多 5×7=35  组

cat("=== 分组信息 ===\n")
all_datasets <- list(
  # 单列
  d_studytime  = d_studytime,
  d_reason     = d_reason,
  d_goout      = d_goout,
  d_parent_edu = d_parent_edu,
  # 两两交互
  d_studytime_reason     = d_studytime_reason,
  d_studytime_goout      = d_studytime_goout,
  d_studytime_parent_edu = d_studytime_parent_edu,
  d_reason_goout         = d_reason_goout,
  d_reason_parent_edu    = d_reason_parent_edu,
  d_goout_parent_edu     = d_goout_parent_edu
)
