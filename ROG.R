...
Comp=function(N_all,p, R, Var.e, nloop, n, dist_x="case1", dist_a="N.ori",groupsize,obj.c=0.5){
  big_column_vector<-c()
  beta=rep(1, p)
...
Comp_RS=function(N_all,p, R, Var.e, nloop, n, dist_x="case1", dist_a="N.ori",groupsize,obj.c=0.5){
  ...
  for (j in 1:lrs) {
    ...
    for (k in 1:nloop) {
      ...
      random.beta= runif(R, min = -1, max = 1)
      ...
      for(i in 1:R){
        ...
        # 第 i 组随机截距
        a.i <- rnorm(1, mean = 0, sd = sigma.a)
        
        # 第 i 组随机斜率向量
        b.i <- rnorm(p, mean = 0, sd = sigma.b)
        
        # 误差项
        e.train <- rnorm(C.train[i], mean = 0, sd = sigma.e)
        e.test  <- rnorm(C.test[i], mean = 0, sd = sigma.e)
        ...
        # 生成响应
        FY.train[idx.train] <- a.i +
          FXX.train[idx.train, ] %*% (beta + b.i) + e.train
        
        FY.test[idx.test] <- a.i +
