...
ans <- run_case11_observation(nloop=as.integer(env_num("NLOOP",50)), p=as.integer(env_num("P",50)),
                              N_test=as.integer(env_num("N_TEST",2500)), obs_bins=as.integer(env_num("OBS_BINS",4)),
                              proxy_sd=env_num("PROXY_SD",0.35), Var.a=env_num("VAR_A",2.25), Var.b=vb,
                              Var.e=env_num("VAR_E",9), tau=as.integer(env_num("TAU",5*(as.integer(env_num("P",50))+1L))),
...
saveRDS(ans,file=sprintf("results_observation/case11_%s.rds",model))
