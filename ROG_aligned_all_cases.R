# ROG_aligned.R
# -----------------------------------------------------------------------------
# CIRG / RASC simulation code aligned with the current paper description.
#
# Major differences from the legacy ROG.R:
#   * Residual-Augmented Supervised Clustering (RASC): cluster on [X, lambda*r].
#   * Minimum group size tau defaults to 5*(p+1).
#   * Full empirical IMSPE (Eq. 10) is optimized, not only Tr(M^{-1} W).
#   * Simulated annealing uses steps {-3,-2,-1,+1,+2,+3}.
#   * Independent reference X is used to estimate W/W_k (no test-X leakage).
#   * Soft Gaussian posterior group assignment is used out of sample.
#   * Random-slope prediction uses a_k + z' b_k, not only a scalar intercept.
#   * Train/test observations from the same true group share the same random effects.
#   * LMM estimation iterates to convergence through lmm_fast_aligned.cpp.
#   * Case 5/6 and sigma_b^2 defaults are aligned with the paper definitions.
#
# Important statistical note:
# The C++ estimator is a convergence-based ML-EM implementation.  It is not an
# exact REML optimizer.  The manuscript should therefore say "convergent EM"
# unless a separate REML implementation is used for the reported estimates.
# -----------------------------------------------------------------------------
...
soft_group_weights <- function(Xnew, params) {
  ...
  for (k in seq_len(K)) {
    Rchol <- params$chol[[k]] # upper triangular; t(R)%*%R = Sigma
...
# ============================================================================
# Full Case 1--9 experiment runner
# ============================================================================

# Running this file with source("ROG_aligned_all_cases.R") will execute all
# nine covariate scenarios for both the random-intercept and random-slope
# experiments. Results are saved after every completed case so that completed
# work is preserved if a later case stops unexpectedly.
...
for (case_id in 1:9) {
  ...
  # Refresh the combined checkpoints after every case.
